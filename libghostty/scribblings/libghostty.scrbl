...
@defstruct*[kitty-graphics-snapshot ([generation (integer-in 0 18446744073709551615)] [placements (and/c vector? immutable?)] [images (and/c hash? immutable?)])]{Copied active-screen graphics content coherent with the render rows. Placements preserve native iterator order within this snapshot, but no cross-snapshot order is promised. @racket[images] contains exactly the unique images referenced by placements. Generation stamps are process-wide content identities; scrolling and resize can change geometry without changing them.}

@defstruct*[kitty-graphics-placement ([image-id (integer-in 0 4294967295)] [placement-id (integer-in 0 4294967295)] [virtual? boolean?] [x-offset (integer-in 0 4294967295)] [y-offset (integer-in 0 4294967295)] [z (integer-in -2147483648 2147483647)] [layer (or/c 'below-background 'below-text 'above-text)] [render-info kitty-graphics-render-info?] [grid-rectangle (or/c #f kitty-graphics-grid-rectangle?)])]{One copied placement. @racket['below-background] means @racket[z] is below -1,073,741,824; @racket['below-text] means it is at least -1,073,741,824 and below zero; @racket['above-text] means it is nonnegative. Viewport and grid geometry are recomputed on every render. A virtual or discarded placement has no grid rectangle. Same-z order across snapshots is unspecified.}

@defstruct*[kitty-graphics-image ([id (integer-in 0 4294967295)] [number (integer-in 0 4294967295)] [width (integer-in 0 4294967295)] [height (integer-in 0 4294967295)] [format (or/c 'rgb 'rgba 'gray-alpha 'gray)] [generation (integer-in 0 18446744073709551615)] [data-length (integer-in 0 18446744073709551615)] [pixels (or/c #f (and/c bytes? immutable?))])]{Decoded, decompressed copied image data. @racket[pixels] is @racket[#f] while a native payload is pending; @racket[data-length] remains its expected decoded size. Stored PNG or compressed state is rejected as a native invariant violation. Unchanged nonpending pixels may be shared between immutable snapshots through a private generation cache.}

@defstruct*[kitty-graphics-render-info ([pixel-width (integer-in 0 4294967295)] [pixel-height (integer-in 0 4294967295)] [grid-columns (integer-in 0 4294967295)] [grid-rows (integer-in 0 4294967295)] [viewport (or/c #f kitty-graphics-viewport-position?)] [source-rectangle kitty-graphics-source-rectangle?])]{Resolved render geometry. @racket[viewport] is absent for virtual or fully off-screen placements; partially visible positions may be negative.}

@defstruct*[kitty-graphics-viewport-position ([column (integer-in -2147483648 2147483647)] [row (integer-in -2147483648 2147483647)])]{Signed viewport-relative placement origin.}

@defstruct*[kitty-graphics-source-rectangle ([x (integer-in 0 4294967295)] [y (integer-in 0 4294967295)] [width (integer-in 0 4294967295)] [height (integer-in 0 4294967295)])]{Resolved and image-clamped source pixel rectangle.}

@defstruct*[kitty-graphics-grid-rectangle ([screen (or/c 'primary 'alternate)] [start terminal-grid-point?] [end terminal-grid-point?])]{Copied screen-space placement bounds with inclusive start and end cells. Endpoints are valid ordinary Racket values and do not expose native selections or grid references.}
