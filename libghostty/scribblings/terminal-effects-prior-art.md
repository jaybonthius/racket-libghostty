...
Public tests in `libghostty/tests/effects.rkt` cover absent/register/replace/clear/close/root-GC behavior; exact PTY ordering and multi-bell; enquiry/XTVERSION response lifetime under GC; true/false size, color, and DA1/DA2/DA3 wire data; copied title/PWD bytes; clipboard destinations, clear versus public empty representation, every result symbol, malformed/read suppression; notification and progress tags; unknown split input, limits, truncation, clearing, disabled capture, and malformed termination; arbitrary and `#f` raised-value identity, first-only fallback behavior, cleanup, stale-state clearing, continued use, same-terminal rejection, uncontended cross-terminal calls, and nested callback cycles. A bounded public two-thread A→B/B→A stress test accepts either an uncontended success or the deterministic lock-unavailable failure and proves every operation completes. It does not force a deterministic inversion because coordinating that schedule would require blocking or descheduling inside an FFI callback, which Racket runs in atomic mode and handlers expressly forbid. `examples/browser-terminal/tests/main.rkt` proves a real DECRQM response returns through the PTY and a BEL reaches the ordinary SSE projection.
