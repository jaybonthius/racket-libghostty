...
The public API uses one contracted setter per effect, with `#f` clearing. Native callbacks, userdata, raw structs, and unions remain private. All borrowed input becomes immutable copied bytes or immutable tagged structs before user code. Registration roots the new callback before native set, preserves the old root if set fails, and releases roots only after replacement, clear, or terminal free. A thread-local callback guard rejects same-terminal calls before semaphore acquisition. Every other terminal lock acquisition from a callback is also nonblocking: it proceeds only when immediately uncontended and otherwise raises a deterministic lock-unavailable value, preventing cross-terminal callback cycles and lock inversions. Each write or resize has a fresh operation frame: callbacks retain only the first raised Racket value, including `#f`, return empty/default XTVERSION, empty enquiry, false size/color/device, `io-error` clipboard, or no-op void fallbacks, let native processing finish, release response allocations, and re-raise the identical value.

Public tests in `libghostty/tests/effects.rkt` cover absent/register/replace/clear/close/root-GC behavior; exact PTY ordering and multi-bell; exact enquiry/XTVERSION replies and defaults; true/false size, color, and DA1/DA2/DA3 wire data; copied title/PWD bytes; clipboard destinations, clear versus public empty representation, every result symbol, malformed/read suppression; notification and progress tags; unknown split input, limits, truncation, clearing, disabled capture, and malformed termination; arbitrary and `#f` raised-value identity, first-only fallback behavior, stale-state clearing, continued use, same-terminal rejection, uncontended cross-terminal calls, and nested callback cycles. A bounded public two-thread A→B/B→A stress test accepts either an uncontended success or the deterministic lock-unavailable failure and proves every operation completes. It does not force a deterministic inversion because coordinating that schedule would require blocking or descheduling inside an FFI callback, which Racket runs in atomic mode and handlers expressly forbid. Raw response-allocation release is structurally reviewed but not instrumented by the public tests; external ASan or Valgrind validation was not run. `examples/browser-terminal/tests/main.rkt` proves a real DECRQM response returns through the PTY and a BEL reaches the ordinary SSE projection.
...
| `GhosttyTerminalEnquiryFn`, `OPT_ENQUIRY` | returned string | Go/Rust | operation-owned foreign response; exact/fallback tests; Scribble |
| `GhosttyTerminalXtversionFn`, `OPT_XTVERSION` | returned string | Go/Rust | operation-owned foreign response; default/exact tests; Scribble |
