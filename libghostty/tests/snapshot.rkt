...
(test-case "ground snapshot round trip preserves observable terminal state"
  (call-with-terminal #:columns 12
                      #:rows 3
                      (lambda (source)
                        (terminal-write! source #"primary")
                        (terminal-write! source #"\33[?1049h\33[1;31m\316\251\33[0m")
                        ...
                        (dynamic-wind void
                                      (lambda ()
                                        ...
                                        (check-equal? (list (render-snapshot-columns source-view)
                                                            (render-snapshot-rows source-view))
                                                      '(12 3)))
                                      (lambda ()
...
(test-case "malformed, truncated, corrupt, and trailing snapshots are rejected"
  (define encoded
