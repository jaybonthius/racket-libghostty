...
(define (call-with-multi-page-terminal procedure)
  (define terminal (make-terminal 10 3))
  (dynamic-wind void
                (lambda ()
                  (for ([line (in-range 300)])
                    (terminal-write! terminal (string->bytes/utf-8 (format "line-~a\r\n" line))))
                  (terminal-resize! terminal 500 3)
                  (procedure terminal))
                (lambda () (terminal-close! terminal))))

(define (make-large-snapshot)
  (call-with-multi-page-terminal terminal->snapshot-bytes))

(define (make-streamed-large-snapshot)
  (call-with-multi-page-terminal (lambda (terminal)
                                   (define output (open-output-bytes))
                                   (define written (terminal-write-snapshot! terminal output))
                                   (define encoded (get-output-bytes output))
                                   (check-equal? written (bytes-length encoded))
...
(test-case "one-shot port snapshots support short reads, trailers, limits, and containment"
  ...
  (for ([limit '(1 2 3 7)])
    ...
    (dynamic-wind void
                  ...
                  (lambda () (terminal-close! restored))))
  (define limit-input (open-input-bytes unfinished))
  (check-equal?
   (ghostty-error-result (lambda () (snapshot-port->terminal limit-input #:max-continuation-bytes 3)))
   'limit-exceeded)
  (check-false (port-closed? limit-input))
  (define truncated-input (open-input-bytes (subbytes ground 0 20)))
  (check-equal? (ghostty-error-result (lambda () (snapshot-port->terminal truncated-input)))
                'invalid-value)
  (check-false (port-closed? truncated-input))
  (define raised (box 'reader-failure))
  (define failed-closed (box #f))
  (define failed-input
    (open-short-input ground 3 #:raised raised #:fail-after 12 #:closed failed-closed))
  (define observed #f)
  (with-handlers ([(lambda (value) (eq? value raised)) (lambda (value) (set! observed value))])
    (snapshot-port->terminal failed-input))
  (check-eq? observed raised)
  (check-false (port-closed? failed-input))
  (check-false (unbox failed-closed))
  ...
  (check-false (port-closed? special-input))
...
(test-case "streamed incremental decoder copies multi-page progress and preserves trailers"
  (define encoded (make-streamed-large-snapshot))
  (define full-reference (snapshot-port->terminal (open-input-bytes encoded)))
  ...
  (define history-fields
    (list (snapshot-history-primary-rows history) (snapshot-history-alternate-rows history)))
  ...
  (check-regexp-match #rx"line-299" (terminal->plain-text terminal))
  (check-exn exn:fail:contract? (lambda () (terminal-close! terminal)))
  (define first-progress (snapshot-decoder-next! decoder))
  (unless first-progress
    (error 'streamed-incremental-test "multi-page fixture reached FINISH before history"))
  (define first-fields
    (list (snapshot-progress-screen first-progress)
          (snapshot-progress-rows first-progress)
          (snapshot-progress-remaining first-progress)))
  (define progress-values
    (let loop ([result (list first-progress)])
      (define progress (snapshot-decoder-next! decoder))
      (if progress
          (loop (cons progress result))
          (reverse result))))
  (check-true (>= (length progress-values) 2))
  (for ([progress (in-list progress-values)])
  ...
  (check-equal? (map snapshot-progress-remaining progress-values)
                (for/list ([remaining (in-range (sub1 (length progress-values)) -1 -1)])
                  remaining))
  (check-not-eq? (car progress-values) (cadr progress-values))
  (check-equal? (list (snapshot-progress-screen first-progress)
                      (snapshot-progress-rows first-progress)
                      (snapshot-progress-remaining first-progress))
                first-fields)
  (check-equal? (snapshot-decoder-next! decoder) #f)
  (check-equal? (snapshot-decoder-next! decoder) #f)
  (check-equal? (list (snapshot-history-primary-rows (snapshot-decoder-history decoder))
                      (snapshot-history-alternate-rows (snapshot-decoder-history decoder)))
                history-fields)
  (check-equal? (snapshot-decoder-source-offset decoder) (bytes-length encoded))
  (check-equal? (read-bytes 7 input) #"trailer")
  (check-false (port-closed? input))
  (check-equal? (terminal->snapshot-bytes terminal) (terminal->snapshot-bytes full-reference))
  (terminal-close! full-reference)
  (terminal-close! terminal)
...
(test-case "incremental mutation, abandonment, failure, and finalization detach terminals"
  ...
  (define live-reference (snapshot-port->terminal (open-input-bytes encoded)))
  (define live-input (open-input-bytes encoded))
  (define live-decoder (make-snapshot-decoder live-input))
  (define live (snapshot-decoder-ready! live-decoder))
  (terminal-write! live-reference #"live-before-history")
  (terminal-write! live #"live-before-history")
  (define live-progress
    (let loop ([result '()])
      (define progress (snapshot-decoder-next! live-decoder))
      (if progress
          (loop (cons progress result))
          (reverse result))))
  (check-true (>= (length live-progress) 2))
  (check-equal? (terminal->snapshot-bytes live) (terminal->snapshot-bytes live-reference))
  (check-false (port-closed? live-input))
  (terminal-close! live-reference)
  (terminal-close! live)
  (snapshot-decoder-close! live-decoder)
  (define resized-input (open-input-bytes encoded))
  (define resized-decoder (make-snapshot-decoder resized-input))
  (define resized (snapshot-decoder-ready! resized-decoder))
  (terminal-resize! resized 21 3)
  (define resize-progress
    (let loop ([result '()])
      (define progress (snapshot-decoder-next! resized-decoder))
      (if progress
          (loop (cons progress result))
          (reverse result))))
  (check-true (>= (length resize-progress) 2))
  (check-true (for/and ([progress (in-list resize-progress)])
                (zero? (snapshot-progress-rows progress))))
  (check-false (snapshot-decoder-next! resized-decoder))
  (check-false (port-closed? resized-input))
  (terminal-write! resized #"usable")
  (terminal-close! resized)
  (snapshot-decoder-close! resized-decoder)
  (define abandoned-closed (box #f))
  (define abandoned-input (open-short-input encoded 65536 #:closed abandoned-closed))
  (define abandoned-decoder (make-snapshot-decoder abandoned-input))
  ...
  (check-false (port-closed? abandoned-input))
  (check-false (unbox abandoned-closed))
  (terminal-write! abandoned #"abandoned")
  (check-regexp-match #rx"abandoned" (terminal->plain-text abandoned))
  (terminal-close! abandoned)
  (define offset-input (open-input-bytes encoded))
  (define offset-decoder (make-snapshot-decoder offset-input))
  ...
  (check-false (port-closed? offset-input))
  (terminal-close! offset-terminal)
  (define corrupt (bytes-copy encoded))
  (bytes-set! corrupt
              (+ history-offset 20)
              (bitwise-xor #xff (bytes-ref corrupt (+ history-offset 20))))
  (define failed-input (open-input-bytes corrupt))
  (define failed-decoder (make-snapshot-decoder failed-input))
  ...
  (define callback-closed (box #f))
  (define callback-input
    (open-short-input encoded
                      3
                      #:raised raised
                      #:fail-after (+ history-offset 20)
                      #:closed callback-closed))
  (define callback-decoder (make-snapshot-decoder callback-input))
  ...
  (check-false (port-closed? callback-input))
  (check-false (unbox callback-closed))
  ...
  (define port-weak #f)
  (define finalized-port-closed (box #f))
  (let ([input (open-short-input encoded 65536 #:closed finalized-port-closed)])
  ...
  (check-false (unbox finalized-port-closed))
...
(test-case "decoder roots its reader through FINISH and releases it afterward"
  (define encoded (make-large-snapshot))
  (define input-weak #f)
  (define input-closed (box #f))
  (define decoder
    (let ([input (open-short-input encoded 65536 #:closed input-closed)])
  ...
  (check-false (unbox input-closed))
  (terminal-close! terminal)
  (snapshot-decoder-close! decoder)
  (define close-weak #f)
  (define close-closed (box #f))
  (define close-decoder
    (let ([input (open-short-input encoded 65536 #:closed close-closed)])
  ...
  (check-false (unbox close-closed))
  (define failed-weak #f)
  (define failed-decoder #f)
  (define failed-closed (box #f))
  (define raised (box 'root-failure))
  (let ([input (open-short-input encoded 3 #:raised raised #:fail-after 12 #:closed failed-closed)])
...
(define (wait-for-callback-stage entered worker stage)
  (unless (sync/timeout 10 entered)
    (kill-thread worker)
    (error 'snapshot-callback-test "timed out waiting for ~a" stage)))
...
(define (check-blocked-snapshot-break make-operation)
  ...
  (wait-for-callback-stage entered worker 'blocked-I/O-callback)
  (break-thread worker)
...
(test-case "snapshot reader rejects decoder reentry and unavailable cross-object locks"
  ...
  (wait-for-callback-stage locked-entered locked-worker 'locked-terminal-writer)
  (define cross-output
  ...
  (wait-for-callback-stage decoder-entered decoder-worker 'locked-snapshot-decoder)
